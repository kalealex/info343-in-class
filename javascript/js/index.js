...
let x = 'this is a literal string';
...
//let "lets us stay at the block level"
//scope is restricted to the block where the variable is declared
...
let junk = ["name",10,null, undefined,{}]; //arrays can contain a variety of types
...
let courses = ["INSC 500", "INSC 570", "INSC 599"];
courses.push("INFO 343");
...
function randomIntegers(amount, max) {
    ...
    for (let i = 0; i < amount; i++) {
        num.push(Math.round(Math.random() * max));
    }
    return num;
}
...
let reverseString = reverseArray(stringToReverse.split("")).join("");
console.log(stringToReverse + " reversed is " + reverseString);
...
function negateArray(arrayOfNumbers) {
    ...
    let negatedArray = [];
    for (let i = 0; i < arrayOfNumbers.length; i++) {
        negatedArray[i] = negate(arrayOfNumbers[i]);
    }
    return negatedArray;
}
...
console.log("compareNums(2,1):", compareNums(2,1)); // this line was wrong in the original file

//some numbers to sort
let numsToSort = [20,1,14,2,21,10,12,11];
console.log("numbers sorted alphabetically:", numsToSort.sort());
console.log("numbers sorted numerically:", numsToSort.sort(compareNums));

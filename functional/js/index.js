...
function descending(comparator) {
    //return new function which negates the result of the original comparator
    return function(record1, record2) {
        return -comparator(record1,record2);
    }
}

//new function which is the negated byCount comparator
...
function pluck(propName) {
    //return a transformer function which returns jusr the requested property
    return function(record) {
...
let someNums = randomIntegers(10,72);
console.log("here are some random numbers :",someNums);
...
let sumNum = someNums.reduce(sum,0);
console.log("the sum of these numbers =",sumNum);
...
let maxNum = someNums.reduce(max,0);
